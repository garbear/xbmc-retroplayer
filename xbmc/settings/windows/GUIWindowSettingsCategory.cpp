...
{
  CSettingsCategory *gamesdir = pSettingsGroup->AddCategory("gamesdir", 15029); // Directories
  if (!gamesdir)
    return;
  ...
  for (VECADDONS::const_iterator it = gameclients.begin(); it != gameclients.end(); it++)
  {
    ...
    // 15026: "External system directory"
    // 15027: "Choose system directory"
    CSetting *path = new CSettingPath(i++, "gamesdir.dir." + id, 15026, dir.c_str(), BUTTON_CONTROL_PATH_INPUT, true, 15027);
    if (path)
