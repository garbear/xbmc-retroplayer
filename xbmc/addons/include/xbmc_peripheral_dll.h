...
extern "C"
{
   ...
   *
   *    * buttons:  Element with state pressed/unpressed
   *
   *    * hats:     Directional pad, opposing buttons are mutually exclusive
   *                yielding 9 states (cardinal/intercardinal directions and
   *                unpressed)
   *
   *    * axes:     Element with state from -1.0 to 1.0, inclusive
   *
   * Joystick peripherals translate these virtual elements into three physical
   * mechanisms:
   *
   *    * buttons:        Digital (state pressed/unpressed) or Analog
   *                      (pressure-sensitive from 0.0 to 1.0, inclusive)
   *
   *    * triggers:       Digital (state pressed/unpressed) or Analog
   *                      (travel distance from 0.0 to 1.0, inclusive)
   *
   *    * analog sticks:  Can travel in two directions (horizontal displacement
   *                      and vertical displacement from -1.0 to 1.0, inclusive)
   *
   * Various driver phenomena make the life of joystick peripherals difficult:
   *
   *    * pressure-sensitive buttons reported as buttons or axes
   ...
   *    * ambiguous sign of analog stick axes
   *
   * Joystick operations allow the frontend to:
   *
   *    * query the virtual and physical elements of the joystick
   ...
   */
  PERIPHERAL_ERROR PerformJoystickScan(JOYSTICK_CONFIGURATIONS* config);
  void FreeJoysticks(JOYSTICK_CONFIGURATIONS* config);
