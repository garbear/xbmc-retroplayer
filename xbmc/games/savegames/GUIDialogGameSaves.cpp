  ...
  case GUI_MSG_WINDOW_INIT:
    {
      ...
      if (g_application.IsPlayingGame())
      {
        ...
        CRetroPlayer *retroplayer = dynamic_cast<CRetroPlayer*>(g_application.m_pPlayer);
        if (message.GetNumStringParams())
        {
          CStdString pathParam(message.GetStringParam());
          if (pathParam.Equals(g_application.CurrentFile()) ||
                (retroplayer && pathParam.Equals(retroplayer->GetFilePath())))
          {
            m_gamePath = retroplayer->GetFilePath();
            m_gameClient = retroplayer->GetGameClient();
          }
          else
          {
...
bool CGUIDialogGameSaves::OnContextMenu(const CFileItemPtr &savefile)
{
  ...
  choices.Add(0, 15028); // Load save state
...
void CGUIDialogGameSaves::CreateSaveState()
{
  ...
  {
    CRetroPlayer *retroplayer = dynamic_cast<CRetroPlayer*>(g_application.m_pPlayer);
    if (retroplayer && retroplayer->Save(label))
...
void CGUIDialogGameSaves::LoadSaveState(const CFileItemPtr &savefile)
{
  ...
  CRetroPlayer *retroplayer = dynamic_cast<CRetroPlayer*>(g_application.m_pPlayer);
  if (g_application.IsPlayingGame() && retroplayer)
  {
    // If game path and game client match, just load the save state
    if (m_gamePath.Equals(retroplayer->GetFilePath()) &&
        m_gameClient.Equals(retroplayer->GetGameClient()))
    {
      if (retroplayer->Load(savefile->GetPath()))
        Close();
      return;
    }
  }
