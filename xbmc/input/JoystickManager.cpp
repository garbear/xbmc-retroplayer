...
void CJoystickManager::ProcessAxisMotion(SJoystick &oldState, const SJoystick &newState, unsigned int joyID)
{
  ...
  {
    // Absolute magnitude
    float absAxis = JOY_ABS(newState.axes[i]);

    // Only send one "centered" message
    if (absAxis < 0.01f)
    {
      if (JOY_ABS(oldState.axes[i]) < 0.01f)
      {
    ...
    if (!CButtonTranslator::IsAnalog(actionID))
    {
      ...
      bool bContinue = !((JOY_ABS(oldState.axes[i]) >= AXIS_DIGITAL_DEADZONE) ^ (absAxis >= AXIS_DIGITAL_DEADZONE));
      oldState.axes[i] = newState.axes[i];
      if (bContinue)
        continue;
