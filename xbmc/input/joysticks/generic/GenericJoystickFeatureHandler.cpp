...
bool CGenericJoystickFeatureHandler::OnButtonPress(JoystickFeatureID id)
{
  ...
  if (buttonKeyId != 0)
  {
    CAction action(CButtonTranslator::GetInstance().GetAction(g_application.GetActiveWindowID(), CKey(buttonKeyId, 0)));
    if (action.GetID() > 0)
    {
      if (action.IsAnalog())
      {
...
void CGenericJoystickFeatureHandler::OnTimeout(void)
{
  ...
  {
    CAction action(CButtonTranslator::GetInstance().GetAction(g_application.GetActiveWindowID(), CKey(m_lastButtonPress, holdTimeMs)));
    if (action.GetID() > 0)
      CApplicationMessenger::Get().SendAction(action);
  }
}
