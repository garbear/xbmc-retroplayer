...
CDirectoryNode* CDirectoryNode::CreateNode(NODE_TYPE Type, const CStdString& strName, CDirectoryNode* pParent)
{
  ...
  case NODE_TYPE_PLATFORM:
    return new CDirectoryNodePlatform(strName, pParent);
  case NODE_TYPE_GAME:
    return new CDirectoryNodeGame(strName, pParent);
  case NODE_TYPE_GENRE:
    return new CDirectoryNodeGenre(strName, pParent);
  case NODE_TYPE_COLLECTION:
    return new CDirectoryNodeCollection(strName, pParent);
  case NODE_TYPE_PUBLISHER:
    return new CDirectoryNodePublisher(strName, pParent);
  case NODE_TYPE_YEAR:
    return new CDirectoryNodeYear(strName, pParent);
  /*
  case NODE_TYPE_RATING:
    return new CDirectoryNodeRating(strName, pParent);
  case NODE_TYPE_MOST_PLAYED:
    return new CDirectoryNodeMostPlayed(strName, pParent);
  case NODE_TYPE_RECENTLY_PLAYED:
    return new CDirectoryNodeRecentlyPlayed(strName, pParent);
  case NODE_TYPE_RECENTLY_ADDED:
    return new CDirectoryNodeRecentlyAdded(strName, pParent);
  */
  default:
    break;
  }
...
void CDirectoryNode::AddQueuingFolder(CFileItemList& items) const
{
  ...
  CGameDbUrl gameUrl;
  if (!gameUrl.FromString(BuildPath()))
    return;
  ...
  */
  ...
  if (pItem)
  {
    bool m_bGameLibraryAllItemsOnBottom = g_advancedSettings.m_bMusicLibraryAllItemsOnBottom; // for now
    pItem->m_bIsFolder = true;
    pItem->SetSpecialSort(m_bGameLibraryAllItemsOnBottom ? SortSpecialOnBottom : SortSpecialOnTop);
    pItem->SetCanQueue(false);
    pItem->SetLabelPreformated(true);
    if (m_bGameLibraryAllItemsOnBottom)
      items.Add(pItem);
    else
