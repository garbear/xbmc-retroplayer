...
std::string CDynamicDatabase::MakeForeignKeyName(const std::string &primary, const std::string &foreign)
{
  return "FK_" + foreign + "_" + primary; // FK_foreign_primary
}

std::string CDynamicDatabase::MakeForeignKeyClause(const std::string &primary, const std::string &foreign)
{
...
std::string CDynamicDatabase::MakeIndexName(const std::string &table, const std::string &column)
{
  return "idx_" + table + "_" + column; // idx_table_column
}

std::string CDynamicDatabase::MakeIndexClause(const std::string &table, const std::string &column)
{
  return "INDEX " + MakeIndexName(table, column) + " (id" + column + ")"; // INDEX idx_table_column (idcolumn)
}

std::string CDynamicDatabase::MakeLinkTableName(const std::string &primary, const std::string &secondary)
{
  return primary + "link" + secondary; // primarylinksecondary
}

std::string CDynamicDatabase::MakeUniqueIndexName(const std::string &primary, const std::string &secondary, int ord)
{
...
std::string CDynamicDatabase::MakeUniqueIndexClause(const std::string &primary, const std::string &secondary, int ord)
{
  if (ord == 1)
  {
...
bool CDynamicDatabase::IsText(std::string type)
{
...
bool CDynamicDatabase::IsFloat(std::string type)
{
  StringUtils::ToUpper(type);
...
bool CDynamicDatabase::IsBool(std::string type)
{
  StringUtils::ToUpper(type);
  return (type.substr(0, 4) == "BOOL");
}

bool CDynamicDatabase::IsInteger(std::string type)
{
  StringUtils::ToUpper(type);
...
CVariant CDynamicDatabase::FieldAsVarient(const field_value &fv, const std::string &type)
{
  if (IsText(type))
...
std::string CDynamicDatabase::PrepareVariant(const CVariant &value, const std::string &type)
{
  if (IsText(type))
...
void CDynamicDatabase::CreateIndex(const std::string &table, const std::string &column, bool isFK)
{
...
void CDynamicDatabase::CreateUniqueLinks(const std::string &primary, const std::string &secondary)
{
...
void CDynamicDatabase::CreateAnalytics()
{
...
int CDynamicDatabase::AddOneToManyItem(const std::string &parent, const std::string &type, const CVariant &var)
{
...
void CDynamicDatabase::AddLink(const std::string &item, int idObject, int idItem)
{
  std::string strSQL;
  strSQL = PrepareSQL(
    "SELECT id%s, id%s "
...
std::string CDynamicDatabase::GetType(const std::string &column)
{
...
bool CDynamicDatabase::GetObjectByIndex(const std::string &column, const CVariant &value, IDeserializable *obj)
{
  if (NULL == m_pDB.get()) return false;
  if (NULL == m_pDS.get()) return false;
...
bool CDynamicDatabase::GetObjectsNav(CFileItemList &items, const std::map<std::string, int> &predicates)
{
  if (NULL == m_pDB.get()) return false;
  if (NULL == m_pDS.get()) return false;
...
bool CDynamicDatabase::GetItemByID(const std::string &itemTable, int idItem, CVariant &value)
{
  if (NULL == m_pDB.get()) return false;
  if (NULL == m_pDS.get()) return false;
...
// Includes parents (one-to-many) and many-to-many tables
bool CDynamicDatabase::GetItemID(const std::string &itemTable, const CVariant &value, int &idItem)
{
  if (NULL == m_pDB.get()) return false;
  if (NULL == m_pDS.get()) return false;
...
bool CDynamicDatabase::GetItemNav(const char *column, CFileItemList &items, const std::string &strPath,
...
unsigned int CDynamicDatabase::Count(std::string column /* = "" */)
{
